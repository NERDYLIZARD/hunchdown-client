...
describe('<BoxPreviewHeader />', () => {
  ...
  it('always renders a div as wrapper', () => {
    const divs = boxPreviewHeader().find('.box-preview__header');
    expect(divs.length).toBe(1);
  });

  it('calls `onEdit()` with the clicked box when `edit` link is clicked', () => {
    const deleteButton = boxPreviewHeader().find('.box-preview__header__edit-button');
  ...
  it('calls `onDelete()` with the clicked box when `delete` link is clicked', () => {
    const deleteButton = boxPreviewHeader().find('.box-preview__header__delete-button');
    deleteButton.simulate('click');
